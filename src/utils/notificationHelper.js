...
import { HelperMethods, SendEmail } from '.';
...
class Notification {
   ...
   */
  static async newTripRequest(req, res, notificationParams) {
    try {
      const { id, dataValues, type } = notificationParams;
      // move all db querying into the controller that will use this method
      ...
      if (user.id) {
        ...
        if (manager.id) {
          ...
          // move this into the Request controller
